...
int main(int argc, char** argv)
{
    ...
    // visualise: convert flow magnitude to a grayscale image
    unsigned char* d_out;
    CUDA_CHECK(cudaMalloc(&d_out, n_gray));
    flow_to_image<<<grid, block>>>(d_u, d_v, d_out, width, height, 10.0f);
    CUDA_CHECK(cudaDeviceSynchronize());

    unsigned char* h_out = (unsigned char*)malloc(n_gray);
    CUDA_CHECK(cudaMemcpy(h_out, d_out, n_gray, cudaMemcpyDeviceToHost));

    stbi_write_png(out_path, width, height, 1, h_out, width);
    printf("Saved: %s\n", out_path);
